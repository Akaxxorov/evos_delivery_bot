...
from keyboards.default import menu2nd, menu, make_locations_kb, make_categories_kb, make_products_kb, number , payments ,tastiqlash
...
@dp.message_handler(text="⬅️ Ortga", state=UserState.nomer)
async def go_back(xabar: Message, state: FSMContext):
    data = await state.get_data()
...
@dp.message_handler(content_types=ContentTypes.CONTACT, state=UserState.nomer)
...
async def zakaz_ql(xabar : Message):
    tg_id = xabar.from_user.id
    menu, text = cart_inline_products(tg_id)
    await xabar.answer(text=text, reply_markup=tastiqlash)
    await UserState.tastiqlash.set()
